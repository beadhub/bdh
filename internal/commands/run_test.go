...
func TestSelectRunDispatchPriority(t *testing.T) {
	...
	if !strings.Contains(chatDecision.Prompt, "Respond to chat from grace") {
	...
	if !strings.Contains(mailDecision.Prompt, "unread mail from mia") {
	...
	if !strings.Contains(claimDecision.Prompt, "Continue working on bdh-421.3: Dispatch logic") {
		t.Fatalf("expected current-claim prompt, got: %q", claimDecision.Prompt)
	}
	if !strings.Contains(claimDecision.Prompt, "code-reviewer pass") {
	...
	if !strings.Contains(readyDecision.Prompt, "Pick up bdh-54y: List active") {
	...
	if !strings.Contains(idleDecision.Prompt, "Check in with your coordinator") {
...
func TestRunLoopUsesDispatcherAfterInitialPrompt(t *testing.T) {
	...
	if !strings.Contains(strings.Join(commands[0], " "), "initial prompt") {
		t.Fatalf("expected first run to use explicit prompt, got %q", strings.Join(commands[0], " "))
	}
