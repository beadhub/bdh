...
func TestRunScreenViewKeepsFirstWrappedInputLineVisible(t *testing.T) {
	model := newRunScreenModel(
		runScreenSnapshot{
	...
	updated, _ := model.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune(strings.Repeat("x", 15))})
	...
	view := model.View()
	if !strings.Contains(view, strings.Repeat("x", 12)) {
		t.Fatalf("expected first wrapped line to remain visible, got %q", view)
	}

	continuation := "\n" + strings.Repeat(" ", lipgloss.Width(model.promptLabel)) + "xx"
